...
import { Award, Users, FileText, BookOpen, Bell, Settings, LayoutDashboard, LogOut, Menu, X, Shield, UserCheck } from "lucide-react";
import { useState } from "react";

const NAV = [
  { href: "/admin", label: "Dashboard", icon: LayoutDashboard },
  { href: "/admin/certificacoes", label: "Certificações", icon: Award },
  { href: "/admin/candidatos", label: "Candidatos", icon: Users },
  { href: "/admin/avaliadores", label: "Avaliadores", icon: UserCheck },
  { href: "/admin/bancas", label: "Bancas", icon: Shield },
  { href: "/admin/cursos", label: "Cursos", icon: BookOpen },
  { href: "/admin/eventos", label: "Eventos / Log", icon: Bell },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30">
        <div className="text-center max-w-sm p-8 bg-white rounded-2xl shadow-lg">
          ...
          <p className="text-muted-foreground mb-6">Acesso restrito. Faça login para continuar.</p>
    ...
    <div className="min-h-screen bg-muted/20 flex">
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-primary text-primary-foreground flex flex-col transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:flex`}>
        <div className="p-5 border-b border-white/10">
          ...
          <p className="text-white/60 text-xs mt-1">Painel Administrativo</p>
        </div>
        ...
        <div className="p-4 border-t border-white/10">
          <div className="text-xs text-white/60 mb-1 truncate">{user?.name}</div>
          <div className="text-xs text-amber-300 mb-2">{user?.role}</div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-border px-4 h-14 flex items-center justify-between lg:hidden">
          <span className="font-bold text-primary">ANEFAC Admin</span>
