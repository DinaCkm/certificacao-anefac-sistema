...
import { Award, FileText, BookOpen, LayoutDashboard, LogOut, Menu, X } from "lucide-react";
import { useState } from "react";
import { trpc } from "@/lib/trpc";

const NAV = [
  { href: "/candidato", label: "Meu Processo", icon: LayoutDashboard },
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-muted/30">
        <div className="text-center max-w-sm p-8 bg-white rounded-2xl shadow-lg">
          ...
          <h2 className="text-2xl font-bold text-primary mb-2">Área do Candidato</h2>
          <p className="text-muted-foreground mb-6">Faça login para acessar seu processo de certificação.</p>
          <a href={getLoginUrl()}><Button size="lg" className="w-full bg-accent text-accent-foreground hover:bg-amber-500">Entrar</Button></a>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-muted/20 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-primary text-primary-foreground flex flex-col transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:flex`}>
        <div className="p-5 border-b border-white/10">
          ...
          <p className="text-white/60 text-xs mt-1">Área do Candidato</p>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="bg-white border-b border-border px-4 h-14 flex items-center justify-between lg:hidden">
          <span className="font-bold text-primary">ANEFAC</span>
